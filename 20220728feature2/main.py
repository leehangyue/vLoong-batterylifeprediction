...
class Main:
    ...
    def run(self):
        ...
        # # Full
        # print("Full")
        # features_full = dataset.get_feature()
        ...
        # # Discharge
        # print("\nDischarge")
        # dataset.regression_type = "discharge"
        # features_discharge = dataset.get_feature()
        ...
        # # Variance
        # print("\nVariance")
        # dataset.regression_type = "variance"
        # features_variance = dataset.get_feature()
        ...
        dataset.regression_type = "mycomb"
        features = dataset.get_feature()
        mode_variance = Train(self.args)
        mode_variance.regression(features, alpha_train=self.args.alpha_train,
                                    regression_type=dataset.regression_type,
                                    l1_ratio=self.args.l1_ratio, log_target=self.args.log_target, model="elastic")

        Eval(self.args, model=mode_variance.model).evaluation(features,
                                   regression_type=dataset.regression_type,
                                   log_target=self.args.log_target)
